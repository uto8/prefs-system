...
export default async function Revenue() {
  ...
  const issuesCount: {officeName: string; data: {month: string; issueCount: number; }[]}[] = await ApiGet("/revenu_calendars/issue_confirms_count")
        ...
        <table className="min-w-full divide-y divide-gray-300">
          ...
          <tbody className="divide-y divide-gray-200 bg-white">
            {issuesCount.map((issue, index) => {
              const issuesMap = issue.data.reduce<{ [key: string]: number }>((acc, { month, issueCount }) => {
                acc[month] = issueCount;
                return acc;
              }, {});
