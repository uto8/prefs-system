...
export default async function Revenue() {
  ...
  const issuesCount: {officeName: string; data: {month: string; totalOrderCheckValue: number; }[]}[] = await ApiGet("/revenu_calendars/own_total_order_check_value")
  return (
    <div>
      <div className="inline-block min-w-full py-2 align-middle sm:px-6 lg:px-8">
        <TitleComponent title="自社工事売上"/>
        <table className="min-w-full divide-y divide-gray-300">
          ...
          <tbody className="divide-y divide-gray-200 bg-white">
            {issuesCount.map((issue, index) => {
              const issuesMap = issue.data.reduce<{ [key: string]: number }>((acc, { month, totalOrderCheckValue }) => {
                acc[month] = totalOrderCheckValue;
                return acc;
              }, {});
