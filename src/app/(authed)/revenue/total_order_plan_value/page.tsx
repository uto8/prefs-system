...
export default async function Revenue() {
  ...
  const issuesCount: {officeName: string; data: {month: string; totalOrderPlanValue: number; }[]}[] = await ApiGet("/revenu_calendars/total_order_plan_value")

  return (
    <div>
      <div className="inline-block min-w-full py-2 align-middle sm:px-6 lg:px-8">
        <TitleComponent title="予定出金額"/>
        <table className="min-w-full divide-y divide-gray-300">
          ...
          <tbody className="divide-y divide-gray-200 bg-white">
            {issuesCount.map((issue, index) => {
              const issuesMap = issue.data.reduce<{ [key: string]: number }>((acc, { month, totalOrderPlanValue }) => {
                acc[month] = totalOrderPlanValue;
                return acc;
              }, {});
