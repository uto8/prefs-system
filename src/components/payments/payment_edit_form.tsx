...
import { editPayment } from '@/app/(authed)/payment/list/actions';
import { useAppDispatch } from '@/stores';
import { updatePayment } from '@/stores/reducers/paymentReducer';
...
export default function PaymentEditForm({open, setOpen, payment}: {
  ...
  const handleSubmit = async (data: z.infer<typeof formSchema>) => {
    try{
      if(!payment) return
      await editPayment({paymentId: payment.id, body:{
          type: data.type,
              ...
              <FormField
                ...
                render={({field}) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Select defaultValue={payment?.type} onValueChange={field.onChange}>
