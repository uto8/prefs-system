...
import { editOrder } from '@/app/(authed)/payment/list/actions';
import { useAppDispatch } from '@/stores';
import { updateOrder } from '@/stores/reducers/orderReducer';
...
export default function OrderEditForm({paymentOpen, setPaymentOpen, editOrderFormDefault}: {
  paymentOpen: boolean;
  setPaymentOpen: Dispatch<SetStateAction<boolean>>;
  editOrderFormDefault: Order | null
}) {
  ...
  useEffect(() => {
    if (editOrderFormDefault) {
      form.reset({
        supplier: editOrderFormDefault.supplier,
        type: editOrderFormDefault.type,
        orderPlanValue: String(editOrderFormDefault.orderPlanValue),
        withdrawalPlanDate: new Date(editOrderFormDefault.withdrawalPlanDate),
        description: editOrderFormDefault.description,
      });
    }
  },[editOrderFormDefault])

  const dispatch = useAppDispatch();

  const handleSubmit = async (data: z.infer<typeof formSchema>) => {
    try{
      if(!editOrderFormDefault)return
      await editOrder({
        orderId: editOrderFormDefault?.id,
        body:{
      ...
      dispatch(updateOrder({
        ...
        type: data.type,
        description: data.description,
        orderChecks: editOrderFormDefault.orderChecks,
        id: editOrderFormDefault.id
      }))
                ...
                <FormField
                  ...
                  render={({field}) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Select defaultValue={editOrderFormDefault?.type} onValueChange={field.onChange}>
